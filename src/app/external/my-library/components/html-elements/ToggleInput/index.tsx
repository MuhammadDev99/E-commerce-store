...
export default function ToggleInput({ label, icon, startValue = false, onChange }: { icon?: string, label: string, startValue?: boolean, onChange?: (value: boolean) => void }) {
    useEffect(() => {
    ...
    return (
        <label className={styles.switchItem}>
            ...
            <span className={styles.label}>{icon && <img src={icon} />}{label}</span>
